...
//1337 Max Supply
//10% of all transactions are burned.
contract DecrementCoin is Token{
    ...
    //claiming function for free claim
    ...
    //balanceOf function for getting balance of user
    function balanceOf(address _owner) external view returns (uint256 balance){
    ...
    //function for transfering coins from user to user. Function also burns 10% of the transaction
    //so end user only receives 90% of what is sent.
    function transfer(address _to, uint256 _value) external returns (bool success){
    ...
    //Same thing as transfer() but for approved addresses
    function transferFrom(address _from, address _to, uint256 _value) external returns (bool success){
    ...
    //function for approving a spender.
    function approve(address _spender, uint256 _value) external returns (bool success){
        allowed[msg.sender][_spender]=_value;
        emit Approval(msg.sender,_spender,_value);
        return true;
    }
    //function returns how much approved balance is left.
    function allowance(address _owner, address _spender) external view returns (uint256 remaining){
